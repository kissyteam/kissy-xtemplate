        ...
        'runBlockCommand': function (engine, scopes, options, name, line) {
            ...
            if (!command) {
                if (!options.params && !options.hash) {
                    ...
                    if (property === false) {
//                        console.log("can not find property: '" + name + "' at line " + line);
                        property = '';
                    } else {
                        property = property[0];
                    }
...
//                    console.error("can not find command module: " + name + "' at line " + line);
                    return '';
                }
            }
            var ret = '';
            try {
                ...
                console.error(e.message + ": '" + name + "' at line " + line);
            }
            if (ret === undefined) {
                ret = '';
            }
    ...
    defaultConfig = {
        ...
        loader: function (subTplName) {
            ...
            if (!tpl) {
                console.error('template "' + subTplName + '" does not exist, ' + 'need to be required or used first!');
            }
            return tpl;
        }
...
function XTemplateRuntime(tpl, config) {
    ...
    config.commands = _.merge(config.commands, commands);
    config.utils = utils;
    config.macros = config.macros || {};
    this.config = config;
}

_.mixin(XTemplateRuntime, {
    commands: commands,
