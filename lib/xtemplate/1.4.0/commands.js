...
commands = {
    ...
    include: function (scopes, config) {
        ...
        if (!params || params.length != 1) {
            console.error('include must has one param');
            return '';
        }
        ...
        if (subTplName.charAt(0) == '.') {
            if (myName == 'unspecified') {
                console.error('parent template does not have name' + ' for relative sub tpl name: ' + subTplName);
                return '';
            }
    ...
    'macro': function (scopes, config) {
        ...
        if (config.fn) {
            ...
            if (!macro) {
                ...
                if (!macro) {
                    console.error("can not find macro module:" + name);
                }
            }
            macro.paramNames.forEach(function (p, i) {
