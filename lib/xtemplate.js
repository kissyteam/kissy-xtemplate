...
var supportsMap = {'5.0.0-alpha.10':0, 'kg-4.1.4':1, 'kg-4.2.0':1, 'kg-4.2.1':1};
...
function XTemplate(cfg){
    ...
    if(!fs.existsSync(path.resolve(__dirname, './xtemplate/', cfg.version)) && ! (cfg.version in supportsMap)){
...
XTemplate.prototype._compile = function (tplContent, filePath, inputCharset, outputCharset) {
    ...
    if(versionNumber < 150){
        moduleCode = this._beautify(
                '/** Compiled By KISSY-XTemplate ' + this.cfg.version + ' */\n' +
        ...
        functionName = this._getFunctionName(filePath);
        moduleCode = '/** Compiled By KISSY-XTemplate ' + this.cfg.version + ' */\n' +
            '/*jshint quotmark:false, loopfunc:true, indent:false, asi:true, unused:false, boss:true, sub:true*/\n' +
            this._escodegen(
                    'KISSY.add(function(S,require,exports,module){\n' +
                    ...
                    'return ' + functionName + '\n' +
                    '});');
    }else if(this.cfg.version === '5.0.0-alpha.3'){
        var compiler = require('./kissy/5.0.0-alpha.3/bin/xtemplate');
        moduleCode = compiler.compile(tplContent, filePath, true);
    }else if(this.cfg.version === '5.0.0-alpha.10'){
        var compileModuleCode = require('./kissy/'+(supportsMap[this.cfg.version]||this.cfg.version)+'/compile-module');
        moduleCode = compileModuleCode({
            kwrap: true,
            content:tplContent,
            path: filePath
        });
    }else if(/^kg\-/.test(this.cfg.version)){
        functionName = this._getFunctionName(filePath);
        var compiler = require('./kg/'+this.cfg.version+'/').Compiler;
        moduleCode = compiler.compile(tplContent, filePath, {
            isModule : true
        });
                ...
                fs.stat(src, function(err, stat){
                    if(stat.isDirectory()){
                        ...
                        fs.readFile(src, function(err, moduleCode){
                            if (self.cfg.type === 'xtpl' || src.match(/\.xtpl\.html$/)|| src.match(/\.xtpl$/) || src.match(/-xtpl\.html$/)) {
                                ...
                                var generate = function(destPath){
                                    fs.stat(destPath, function(err, destStat){
                                        ...
                                        if(path.extname(dest) !== '.js' && destStat.isDirectory()){
                                            modulePath = path.resolve(dest, path.basename(src, path.extname(src)).replace(/\.(x?tpl)$/, '-$1') + '.js');
                                        }else{
                                            modulePath = destPath;
                                        }
