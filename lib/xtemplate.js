...
function XTemplate(cfg){
    ...
    if(!fs.existsSync('./xtemplate/' + cfg.version)){
        throw new Error('version ' + cfg.version + ' not supported yet. Please submit a new issue at ');
    }
    cfg.inputCharset = cfg.inputCharset || 'utf8';
    cfg.outputCharset = cfg.outputCharset || 'utf8';
    var compiler = require('./xtemplate/' + cfg.version + '/compiler');

    compiler.forEach(function(fn, name){
...
XTemplate.prototype._compile = function (tplContent, inputCharset, outputCharset) {
    ...
    var moduleCode = this._beautify(
        '/** Compiled By KISSY-XTemplate */\n' +
            ...
            'return ' + this.compileToStr(tplContent)) + ';\n' +
        '});';
                            ...
                            var generate = function(destPath){
                                fs.stat(destPath, function(err, destStat){
                                    ...
                                    if(destStat.isDirectory()){
                                        modulePath = path.resolve(dest, path.basename(src, '.html').replace(/\.(x?tpl)$/, '-$1'));
                                    }else{
                                        modulePath = destPath;
                                    }
        ...
        if(fs.statSync(src).isDirectory()){
            ...
            if(files.length){
                files.forEach(function(filePath){
                    self.compileSync(filePath, dest);
                });
            }
        }else{
